...
* EOA（Externally Owned Account)、外部所有アカウント
  * ユーザーによりコントロールされるアカウントであり、我々ユーザーによる任意のタイミングで、EOAがトランザクションを発生させ、他のアカウントへのetherの送金、コントラクト・コードの実行などを行う
  * 秘密鍵によって制御され、「0x」で始まるウォレットアドレスを持つ仮想通貨ウォレットだと認識されているものの実態
* Contract
  * 一種の自動エージェントであり、EOAにより発生したトランザクションをトリガにContractアカウントが内部に持つコントラクト・コードが実行される
...
ウェブサイトにてNFT（コントラクトはEthereum上にある）をMetamaskを利用して購入する場合。

1. ウェブサイトへアクセス

2. Metamask（ウォレット）への接続: ウェブサイトは、Metamaskと接続するためのリクエストを行います。Metamaskのウィンドウが表示され、ウェブサイトへの接続許可を求められます。許可することで、ウェブサイトはユーザーのアドレスを取得できます。

3. 購入ボタンをクリック

4. トランザクションの作成: ウェブサイトは、NFTの購入に必要なトランザクションを作成し、Metamaskに送信します。トランザクションには、送信元アドレス（ユーザーのアドレス）、送信先アドレス（NFTのコントラクトアドレス）、送信額（通常はガス代を含む）、およびトランザクションデータ（購入を指示するスマートコントラクトの関数呼び出し情報）が含まれます。

5. トランザクションの確認: Metamaskは、トランザクションの詳細を表示し、ユーザーに確認を求めます。内容を確認して問題がなければ、承認ボタンをクリックします。

6. トランザクションの署名: 承認をクリックすると、Metamaskは秘密鍵を使ってトランザクションに署名します。これにより、ユーザーがトランザクションの送信者であることが証明されます。秘密鍵はMetamask内で安全に保管され、ウェブサイトや第三者には共有されません。

7. トランザクションの送信: 署名されたトランザクションは、Ethereumネットワークに送信され、マイナーによって処理されるための待機キューに入ります。

8. トランザクションの処理: マイナーがトランザクションを取り上げ、ブロックに追加してブロックチェーンにマイニングすると、トランザクションは完了します。これにより、スマートコントラクトが実行され、NFTがユーザーのウォレットアドレスに追加されます。

9. トランザクション成功: スマートコントラクトにてイベント送信処理を設定しておくことで、外部のアプリケーションがそれを購読することがかのうです。スマートコントラクトの処理が正常に終了した場合にイベントを送信することでウェブサイトにてユーザーに結果をフィードバックできます。
